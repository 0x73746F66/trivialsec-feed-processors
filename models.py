...
class BinaryDefense(BaseModel, DAL):
    ...
    def load(
        ...
        response = services.aws.get_dynamodb(table_name=services.aws.Tables.EWS_BINARY_DEFENSE, item_key={'address_id': str(self.address_id)})
        if not response:
    ...
    def save(self) -> bool:
        return services.aws.put_dynamodb(table_name=services.aws.Tables.EWS_BINARY_DEFENSE, item=self.dict())

    def delete(self) -> bool:
        return services.aws.delete_dynamodb(table_name=services.aws.Tables.EWS_BINARY_DEFENSE, item_key={'address_id': str(self.address_id)})
